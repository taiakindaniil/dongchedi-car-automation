...
- `inspection` — отчёт: **0** при подтверждённом ДТП/утопе/пожаре/скрутке;
  **1** если по мобильному API все четыре пункта заключения явно «нет»;
  **≈0.45** при теге 检测报告 в листинге, но выводов Sophon ещё нет;
  **≈0.2** без тега; **промежуточно** при частично известных выводах.
